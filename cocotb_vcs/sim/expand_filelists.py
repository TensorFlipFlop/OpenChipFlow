...
def _parse_filelist(
    path: str, seen_lists: Set[str], out_files: List[str], out_args: List[str]
) -> None:
            ...
            while i < len(parts):
                ...
                if tok in ("-f", "-F"):
                    ...
                    if i >= len(parts):
                        break
                    inc = parts[i]
                    _parse_filelist(_resolve_path(base_dir, inc), seen_lists, out_files, out_args)
                    i += 1
                    continue
                ...
                # plain source file
                out_files.append(_resolve_path(base_dir, tok))
                i += 1
...
def _expand(argv: List[str]) -> Tuple[List[str], List[str]]:
    out_files: List[str] = []
    out_args: List[str] = []
    seen_lists: Set[str] = set()
    for fl in argv:
        _parse_filelist(fl, seen_lists, out_files, out_args)
    return _unique_preserve(out_files), _unique_preserve(out_args)
...
def main(argv: List[str]) -> int:
    ...
    try:
        sources, args = _expand(ns.filelists)
