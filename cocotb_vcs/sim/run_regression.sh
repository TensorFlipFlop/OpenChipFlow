...
base_make=()
for v in PYTHON_BIN TOPLEVEL RTL_FILELISTS TB_SOURCES W TOP_PARAMS WAVES WAVE_SCOPE FSDB_FILE DEBUG QUIET COV COV_TYPES COV_DIR VERDI_HOME VERDI_PLI_TAB VERDI_PLI_LIB; do
  if [[ -n "${!v-}" ]]; then
    base_make+=("${v}=${!v}")
  fi
done
...
for m in "${modules[@]}"; do
  for s in "${seeds[@]}"; do
    ...
    if [[ "${REGR_SAVE_LOGS}" == "1" ]]; then
      extra_make+=(COMPILE_LOG="${OUT_DIR}/compile.log")
      extra_make+=(SIM_LOG="${OUT_DIR}/sim_${idx}.log")
    fi
    ...
    if [[ -z "${case_override}" ]]; then
      ...
      case_override="${toplevel}__${module_tag}"
    fi
    case_make=()
    ...
    cov_dir_override="${REGR_COV_DIR:-}"
    if [[ -z "${cov_dir_override}" ]]; then
      cov_dir_override="${out_root}/${case_override}/cov/cm.vdb"
    fi
    cov_make=(COV_DIR="${cov_dir_override}")
    if [[ "${REGR_DEBUG}" == "1" ]]; then
      echo "[debug] run ${idx}: TOPLEVEL=${toplevel} CASE=${case_override} MODULE=${m} SEED=${s} COV_DIR=${cov_dir_override}" >> "${DEBUG_LOG}"
    fi

    make -C "${SCRIPT_DIR}" "${base_make[@]}" "${case_make[@]}" "${cov_make[@]}" \
      COCOTB_TEST_MODULES="${m}" COCOTB_TESTCASE= TESTCASE= MODULE= \
      SEED="${s}" "${extra_make[@]}" \
      COCOTB_RESULTS_FILE="${OUT_DIR}/results_${idx}.xml"
  done
done
